...
function calcular_parcelamento() {
    ...
    if (forma_pagamento === 2) {
        ...
        <p>A vista, seu procedimento sairá por <b>R$ ${total.toFixed(2)}</b>. 
        `;
        ...
        if (parcela <= 0 || parcela > MAX_PARCELAS) {
            ...
            if (jurosSelecionado[parcela - 1] > jurosOutro[parcela - 1]) {
                ...
                document.getElementById("resultado").innerHTML = `
                    <h2>${document.getElementById("nome").value}</h2>
                    <p>Em <b>${parcela} vezes</b>, seu procedimento de <b>${document.getElementById("cirurgia").value}</b> sairá por <b>R$ ${prazoSelecionado.toFixed(2)}</b> mensais, totalizando <b>R$ ${totalSelecionado.toFixed(2)}</b>.</p>                
                    <p>A taxa de juros dessa transação foi de <b>${jurosPercentSelecionado.toFixed(2)}%</b>, acrescentando R$ <b>${adicionalSelecionado.toFixed(2)}</b> ao valor do procedimento.</p>
                    <p>Na outra máquina, o procedimento pode ser R$ <b>${diferenca.toFixed(2)}</b> mais barato.</p>
                `;
                ...
                document.getElementById("resultado").innerHTML = `
                    ...
                    <p>Em <b>${parcela} vezes</b>, seu procedimento de <b>${document.getElementById("cirurgia").value}</b> sairá por <b>R$ ${prazoSelecionado.toFixed(2)}</b> mensais, totalizando <b>R$ ${totalSelecionado.toFixed(2)}</b>.</p>                
                    <p>A taxa de juros dessa transação foi de <b>${jurosPercentSelecionado.toFixed(2)}%</b>, acrescentando R$ <b>${adicionalSelecionado.toFixed(2)}</b> ao valor do procedimento.</p>
                    <p>Na outra máquina, o procedimento pode ser R$ <b>${diferenca.toFixed(2)}</b> mais caro.</p>
                `;
            } else if (jurosSelecionado[parcela - 1] === jurosOutro[parcela - 1]) {
                document.getElementById("resultado").innerHTML = `
                    <h2>${document.getElementById("nome").value}</h2>
                ...
                fetch('juros2.txt')
                    ...
                    .then(data => {
                        ...
                        document.getElementById("resultado").innerHTML = `
                            <h2>${document.getElementById("nome").value}</h2>
                            <p>Em <b>${parcela} vezes</b>, seu procedimento de <b>${document.getElementById("cirurgia").value}</b> sairá por <b>R$ ${prazo.toFixed(2)}</b> mensais, totalizando <b>R$ ${total.toFixed(2)}</b>.</p>                
                            <p>A taxa de juros dessa transação foi de <b>${juros_percent.toFixed(2)}%</b>, acrescentando R$ <b>${adicional.toFixed(2)}</b> ao valor do procedimento.</p>
